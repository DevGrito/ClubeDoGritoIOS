...
interface Beneficio {
  ...
  // ✅ NOVO
  ciclosPagamento?: string[]; // ex: ["mensal","trimestral","anual"]
...
export default function BeneficioDetalhes() {
  ...
  const { data: beneficio, isLoading: isLoadingBeneficio } = useQuery<Beneficio>({
    ...
    enabled: !!beneficioId,
    select: (data) => {
        const fallback = `/api/beneficios/${data.id}/imagem`;
        return {
          ...data,
          imagem: data.imagem || fallback,
          imagemUrl: data.imagemUrl || fallback,
        };
      },
  });
  ...
  // Inicializar valorLanceCustomizado com o mínimo do benefício quando carregar
  useEffect(() => {
    if (beneficio && valorLanceCustomizado === 0) {
      const minimo = typeof beneficio.pontosNecessarios === 'string' 
        ? parseInt(beneficio.pontosNecessarios.replace(/\D/g, '')) || 100
        : typeof beneficio.pontosNecessarios === 'number' 
          ? beneficio.pontosNecessarios 
          : 100;
      setValorLanceCustomizado(minimo);
    }
  }, [beneficio, valorLanceCustomizado]);
 ...
 const verificarElegibilidade = () => {
  ...
  const planoUsuario = (userData.plano || "eco").toLowerCase();
  ...
  if (beneficio.planosDisponiveis && beneficio.planosDisponiveis.length > 0) {
    const planosValidos = beneficio.planosDisponiveis.map((p) =>
      (p || "").toLowerCase()
    );

    if (!planosValidos.includes(planoUsuario)) {
      const planosNomes = beneficio.planosDisponiveis
        .map((p) => getPlanDisplayName(p))
        .join(", ");
  ...
  // ✅ NOVO: periodicidade
  const periodicidadeUsuario = ((userData as any)?.periodicity || "mensal").toLowerCase();
  const ciclosDoBeneficio = (beneficio.ciclosPagamento || ["mensal"]).map((c) =>
    (c || "").toLowerCase()
  );

  if (!ciclosDoBeneficio.includes(periodicidadeUsuario)) {
    return {
      elegivel: false,
      motivo: `Sua periodicidade (${capitalizeFirstLetter(
        periodicidadeUsuario
      )}) não permite este prêmio. Disponível para: ${ciclosDoBeneficio
      ...
      if (response.success) {
        ...
        console.error("Erro ao dar lance:", error);
          ...
          const msg =
            ...
            "Não foi possível enviar seu lance. Tente novamente.";
