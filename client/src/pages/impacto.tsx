  ...
  export default function Impacto() {
    ...
    // Animar contador de Horas/Aula (usa valor do banco)
    useEffect(() => {
      const targetValue = indicadoresGlobais?.horasAula || 226359;
    ...
    }, [indicadoresGlobais?.horasAula]); // Atualiza quando o valor do banco mudar

    // Animar contador de Impacto Anual (usa valor do banco)
    useEffect(() => {
      const targetValue = indicadoresGlobais?.impactoDiretoIndireto || 317062;
      const duration = 3000; // 3 segundos
      const steps = 60;
      const stepTime = duration / steps;
      const increment = targetValue / steps;
    ...
    // Função para calcular visitas em domicílio (TOTAL ANUAL - sempre anual)
      const getVisitasDomicilio = () => {
        // 1 - tentativa principal: dados ANUAIS do Gestão à Vista
        const valorGestao = gestaoVistaData?.indicadores?.visitas?.valor;
        if (typeof valorGestao === "number" && valorGestao > 0) {
          return valorGestao;
        }

        // 2 - fallback: indicadoresGlobais, caso existam
        const valorGlobal =
          indicadoresGlobais?.visitasDomicilio ||
          indicadoresGlobais?.visitas ||
          indicadoresGlobais?.visitasDomiciliares;
