...
const getFirstName = (fullName: string): string => {
  if (!fullName) return "Alguém";
  const parts = fullName.trim().split(' ');
  return parts[0];
};

// Componente para mostrar alerta de lance maior
const LanceMaiorAlerta = ({ 
  beneficioId, 
  userId, 
  meuLance,
  onAumentarLance 
  ...
  userId: string;
  meuLance: number;
  onAumentarLance: (beneficioId: number) => void;
}) => {
  ...
  if (isLoading || !estatisticas) return null;
  
  // Se o usuário está em primeiro lugar ou não há concorrentes, não mostra alerta
  if (estatisticas.posicaoUsuario === 1 || estatisticas.top3Lances.length === 0) return null;
  
  // Encontrar o líder (1º lugar que não é o usuário)
  const lider = estatisticas.top3Lances.find(l => l.posicao === 1 && !l.isUsuario);
  if (!lider) return null;

  const primeiroNome = getFirstName(lider.nome);
  const diferenca = lider.pontos - meuLance;

  return (
    <motion.div
      initial={{ opacity: 0, y: -10 }}
      animate={{ opacity: 1, y: 0 }}
      className="bg-gradient-to-r from-orange-50 to-red-50 border-2 border-orange-200 rounded-xl p-4 mb-4"
    >
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-semibold text-orange-800">
              {primeiroNome} deu <span className="text-orange-600">{formatNumber(lider.pontos)} Gritos</span>
            </p>
            <p className="text-xs text-orange-600">
              Você precisa de +{formatNumber(diferenca + 1)} para liderar
            </p>
          </div>
        </div>
        <Button
          size="sm"
          onClick={() => onAumentarLance(beneficioId)}
          className="bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white border-0 shadow-md hover:shadow-lg transition-all"
          data-testid={`btn-aumentar-lance-alerta-${beneficioId}`}
        >
...
export default function MeusLances() {
  ...
  // Função para abrir modal de aumentar lance a partir do alerta (sem precisar de estatisticas carregadas)
  const handleAumentarLanceFromAlert = (beneficioId: number) => {
    setLanceDetalhes(beneficioId);
    // O useEffect abaixo vai abrir o modal quando as estatísticas forem carregadas
  };

  // Efeito para abrir modal de aumentar lance quando estatísticas são carregadas
  useEffect(() => {
    if (lanceDetalhes && estatisticas && !showAumentarLance) {
      // Verificar se o usuário não está em primeiro lugar
      if (estatisticas.posicaoUsuario > 1) {
        const melhorLance = estatisticas.top3Lances[0]?.pontos || estatisticas.lanceUsuario.pontos;
        const incrementoMinimo = estatisticas.gritosMinimos || 1;
        const novoLanceSugerido = melhorLance + incrementoMinimo;
        setNovoLanceValor(novoLanceSugerido);
        setShowAumentarLance(true);
      }
    }
  }, [lanceDetalhes, estatisticas, showAumentarLance]);
            ...
            {lances.map((lance, index) => {
              ...
              return (
                ...
                >
                          ...
                          <div className="flex items-center justify-between gap-2 flex-nowrap">
                            ...
                            <div className="flex items-center space-x-1 sm:space-x-3 flex-shrink-0 min-w-0">
                              ...
                              <Button
                                ...
                                className="bg-yellow-500 hover:bg-yellow-600 text-black border-0 shadow-sm hover:shadow-md transition-all duration-200 text-xs sm:text-sm px-3 sm:px-4 min-w-[85px] sm:min-w-auto"
                  ...
                  {/* Alerta de lance maior para lances ativos */}
                  {lance.status === 'ativo' && userId && (
                    <LanceMaiorAlerta
                      beneficioId={lance.beneficioId}
                      userId={userId}
                      meuLance={lance.pontosOfertados}
                      onAumentarLance={handleAumentarLanceFromAlert}
                    />
                  )}
