...
import { ScanFace, Lock, User, ArrowLeft } from "lucide-react";
import { syncAuthSessionAfterLogin } from "@/lib/auth-session";
import { queryClient } from "@/lib/queryClient";

export default function TabletChamadaLogin() {
  ...
  const handleLogin = async () => {
    ...
    setErro("");
    try {
      const r = await fetch("/api/tablet-chamada/login", {
        method: "POST",
      ...
      sessionStorage.setItem("tablet_chamada_vertente", d.usuario?.vertente || session.vertente || "");
      sessionStorage.setItem("tablet_chamada_user", d.usuario?.username || "");
      await queryClient.invalidateQueries({ queryKey: ["/api/tablet-chamada/turmas"] });
      setLocation("/tablet/chamada");
    } catch {
      ...
      <div className="w-full max-w-md bg-gray-800 border border-gray-700 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-8">
          <div className="text-center mb-6">
            <div className="mx-auto w-16 h-16 bg-yellow-400/10 border border-yellow-400/30 rounded-full flex items-center justify-center mb-4">
              <ScanFace className="w-8 h-8 text-yellow-400" />
            </div>
            <h1 className="text-2xl font-bold text-white">Chamada O Grito</h1>
            <p className="text-gray-400 text-sm mt-1">Tablet — porta da sala</p>
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-10 bg-gray-700 border-gray-600 text-white placeholder:text-gray-500 focus:border-yellow-400 focus:ring-yellow-400/20"
                  autoCapitalize="none"
                  autoCorrect="off"
                />
              </div>
            ...
            <button
              onClick={handleLogin}
              disabled={loading}
              className="w-full py-3 rounded-xl bg-yellow-400 hover:bg-yellow-300 disabled:opacity-50 text-black font-semibold transition-colors"
            >
