...
export default function Entrar() {
  ...
  const handleEnviarCodigo = async () => {
    ...
    setIsLoading(true);
    try {
      // ✅ VERIFICAR ELEGIBILIDADE ANTES DE ENVIAR SMS
      console.log("🔍 [LOGIN] Verificando elegibilidade para:", fullPhone);
      ...
      console.log("🔍 [LOGIN] Resposta elegibilidade:", eligibility);
      
      // Se não for elegível, bloquear login
      if (!eligibility.eligible) {
        console.log("❌ [LOGIN] Usuário não elegível:", eligibility.reason);
        
        if (eligibility.reason === "SUBSCRIPTION_INACTIVE") {
          // ✅ PERMITIR LOGIN - salvar dados para mostrar tela de reativação após login
          localStorage.setItem("userId", String(eligibility.userId));
          localStorage.setItem("userName", eligibility.userName || "");
          localStorage.setItem("userPhone", fullPhone);
          localStorage.setItem("userTelefone", fullPhone);
          localStorage.setItem("hasActiveSubscription", "false");
          localStorage.setItem("needsReactivation", "true");
          
          console.log("⚠️ [LOGIN] Assinatura inativa, mas permitindo login para reativação");
          // Continuar com o fluxo normal de login (enviar SMS)
        } else if (eligibility.reason === "PAYMENT_REQUIRED") {
          toast({
          ...
          toast({
            title: "Acesso negado",
            description: eligibility.message || "Não foi possível verificar seu acesso.",
            variant: "destructive",
          });
        }
        setIsLoading(false);
      ...
      console.log("✅ [LOGIN] Usuário elegível, enviando SMS...");
      ...
      if (data.codigo) {
        ...
        // Mostrar toast com código para qualquer número de teste
        toast({
  ...
  const handleVerificarCodigo = async () => {
    ...
    try {
      ...
      if (
        ...
        // Léo Martins: fluxo especial baseado no status da assinatura
        if (!hasActiveSubscription) {
          // Assinatura inativa → direto para admin com aviso
          console.log("⚠️ ENTRAR.TSX: Leo com assinatura inativa - redirecionando direto para /administrador");
          localStorage.setItem("subscriptionPaused", "true");
          toast({
            title: "Acesso restrito",
            description: "Sua assinatura não está ativa. Você não tem acesso à tela de doador.",
            duration: 5000
          });
          setLocation("/administrador");
        } else {
          // Assinatura ativa → tela de doador (onde tem opção de ir para admin no menu)
          console.log("✅ ENTRAR.TSX: Leo com assinatura ativa - redirecionando para /tdoador");
          setLocation("/tdoador");
        }
        ...
        console.log(
          `🔄 ENTRAR.TSX: Doador - hasActiveSubscription: ${hasActiveSubscription}`
        );
        
        // ✅ Se assinatura NÃO está ativa, redirecionar para página de reativação
        if (!hasActiveSubscription) {
          console.log("⚠️ ENTRAR.TSX: Assinatura inativa - redirecionando para /assinatura-pausada");
          localStorage.setItem("subscriptionPaused", "true");
          toast({
          ...
          console.log("✅ ENTRAR.TSX: Assinatura ativa - redirecionando para /tdoador");
          setLocation("/tdoador");
        }
      } else if (
