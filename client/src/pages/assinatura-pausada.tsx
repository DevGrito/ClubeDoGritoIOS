  ...
  useEffect(() => {
    ...
    pr.on('paymentmethod', async (event) => {
      try {
        ...
        // DUAL ROLE: Limpar flag quando assinatura é reativada
        localStorage.removeItem('isDualRoleUser');
    ...
    return (
      <div className="w-full max-w-sm px-6">
        ...
        <div className="pt-16">
          ...
          {/* DUAL ROLE: Botão para acessar área administrativa (para usuários com papel admin: leo, admin, super_admin) */}
          {['leo', 'admin', 'super_admin'].includes(localStorage.getItem('userPapel') || '') && (
            <Button 
              onClick={() => {
                // Redirecionar para área administrativa (todos vão para /administrador)
                setLocation('/administrador');
              }}
              variant="outline"
              className="w-[90%] mx-auto flex items-center justify-center border-[#FFD700] text-[#B8860B] font-medium py-4 rounded-xl text-base transition-all duration-200 hover:bg-[#FFD700]/10 hover:text-[#B8860B] mt-4"
              data-testid="button-access-admin-area"
            >
