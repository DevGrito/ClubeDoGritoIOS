...
export default function CoordenadorInclusaoPage() {
  ...
  const handleImportExcel = () => setShowImportModal(true);
...
const handleExportTemplate = () => {
  // 1) Cabeçalhos (iguais ao payload do "Criar")
  const programasHeaders = [
    "nome",
    "modalidade",
    "duracao",
    "numeroVagas",
  ...
  const cursosHeaders = [
    ...
    "status",
    "descricao",
    "turmasCodigos", // COD1;COD2
  ];

  const participantesHeaders = [
    "nome",
    "cpf",
    "genero",
    "idade",
  ...
  // 2) Exemplos (1 linha por guia)
  const programasExample = [[
    "Programa Teste",
    "Presencial",
    "3 meses",
    30,
  ...
  const participantesExample = [[
    "Maria da Silva",
    "12345678901",       // IMPORTANTÍSSIMO: deixar como texto
    "Feminino",
    22,
    "MAT-2025-001",
    "ID-001",
    "2025-01-10",
    "maria@email.com",
    "31999999999",       // IMPORTANTÍSSIMO: texto
    "Rua X, 123",
  ...
  // 3) Monta o workbook com guias
  const wb = XLSX.utils.book_new();

  const wsProgramas = XLSX.utils.aoa_to_sheet([programasHeaders, ...programasExample]);
  const wsTurmas = XLSX.utils.aoa_to_sheet([turmasHeaders, ...turmasExample]);
  const wsCursos = XLSX.utils.aoa_to_sheet([cursosHeaders, ...cursosExample]);
  const wsParticipantes = XLSX.utils.aoa_to_sheet([participantesHeaders, ...participantesExample]);
  ...
  // 4) Baixa o arquivo
  XLSX.writeFile(wb, "template_import_inclusao_produtiva.xlsx");
};
 ...
 async function handlePreviewImport() {
  ...
  try {
    ...
    if (!resp.ok) throw new Error(data?.error || "Erro ao ler planilha");

    setImportId(data.importId);
    setPreviewData(data.preview);
    setPreviewStats(data.stats);

    // seleciona tudo válido por padrão
   const allValidParticipantIndexes =
    (data.preview.participantes || [])
      .filter((r: any) => r.isValid)
      .map((r: any) => Number(r.index));

  setSelectedIndexes(allValidParticipantIndexes);

    toast({
      title: "Planilha carregada",
      description: "Programas, turmas, cursos e participantes carregados com sucesso.",
    });
  } catch (e: any) {
    toast({
...
async function handleCommitImport() {
  ...
  toast({
    ...
    description: "Programas, turmas, cursos e participantes importados com sucesso.",
  });

    // opcional: refetch participantes
    // queryClient.invalidateQueries(["participantes-inclusao"])
  } catch (e: any) {
    console.error(e);
  ...
  const previewSafe = previewData ?? { programas: [], turmas: [], cursos: [], participantes: [] };
  if (isLoading) {
    return (
          ...
          <div className="flex items-center gap-3">
              ...
              🎯 Coordenador Inclusão
                 ...
                 <Button
                  ...
                  <Dialog open={showImportModal} onOpenChange={setShowImportModal}>
                   <DialogContent className="sm:max-w-3xl w-[95vw] max-h-[85vh] overflow-hidden flex flex-col">

                      <DialogHeader>
                        <DialogTitle>Importar Participantes – Inclusão Produtiva</DialogTitle>
                        <DialogDescription>
                          Faça upload de um arquivo Excel (.xlsx ou .xls) com os dados dos participantes.
                        </DialogDescription>
                      </DialogHeader>
                      ...
                      <div className="space-y-4 overflow-y-auto pr-2">
                        <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-green-400 transition-colors">
                          ...
                          />
                          <label htmlFor="file-upload-inclusao" className="cursor-pointer">
                      ...
                      <div className="text-sm text-gray-600 space-y-1">
                        <div>
                          Encontrados:{" "}
                          <b>
                            {(["programas", "turmas", "cursos", "participantes"] as const).reduce(
                              (acc, k) => acc + (previewStats[k]?.valid || 0) + (previewStats[k]?.invalid || 0),
                              0
                            )}
                          </b>{" "}
                          linhas
                        ...
                        {(["programas", "turmas", "cursos", "participantes"] as const).map((k) => (
                          <div key={k}>
                            <b className="capitalize">{k}:</b> {previewStats[k]?.valid || 0} válidas •{" "}
                            {previewStats[k]?.invalid || 0} com erro
                          </div>
                        ))}
                      </div>
                    )}
                ...
                {previewData && (
                  <div className="border rounded-lg p-3 bg-green-50 max-h-64 overflow-auto space-y-3">
                    <p className="text-sm font-medium text-green-800">
                      Preview dos dados encontrados (selecione o que importar)
                    </p>

                    {(["programas","turmas","cursos","participantes"] as const).map((key) => (
                      <div key={key} className="border rounded bg-white p-2">
                        <div className="text-sm font-semibold capitalize mb-2">
                          {key} ({previewSafe[key].length})
                        </div>

                        {previewSafe[key].map((r: any) => {
                          const isParticipante = key === "participantes";

                          return (
                            <label key={`${key}-${r.index}`} className="flex items-start gap-2 py-1 border-b last:border-b-0">
                              {isParticipante ? (
                                <input
                                  type="checkbox"
                                  checked={selectedIndexes.includes(Number(r.index))}
                                  disabled={!r.isValid}
                                  onChange={(e) => {
                              ...
                              <div className="flex-1">
                                <div className="text-xs text-gray-500">Linha {Number(r.index) + 2}</div>
                                <div className="text-sm font-medium">
                                  {r.data?.nome || r.data?.codigo || "(sem identificador)"}
                                </div>

                                {!r.isValid && (
                                  <div className="text-xs text-red-600 mt-1">
                                    {r.errors?.map((er: any, i: number) => (
                                      <div key={i}>• {er.path}: {er.message}</div>
                                    ))}
                                  </div>
                                )}
                              </div>
                     ...
                     <DialogFooter className="pt-3 border-t flex flex-col sm:flex-row sm:justify-between gap-2">
                        <Button
                          variant="outline"
                          onClick={() => {
                            setShowImportModal(false);
                        ...
                        >
                          {isImporting ? "Lendo planilha..." : "Ler planilha"}
                        </Button>

                        <Button
                          onClick={handleCommitImport}
                          disabled={!importId || selectedIndexes.length === 0 || isImporting}
                          className="bg-green-600 hover:bg-green-700"
                        >
                          {isImporting ? "Importando..." : `Confirmar (${selectedIndexes.length})`}
                        </Button>
                      </DialogFooter>
                    </DialogContent>
                  </Dialog>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48">
                <DropdownMenuItem
                  onClick={() =>
                    window.open("https://monday.com/lang/pt", "_blank")
                  }
                ...
                <DropdownMenuItem
                  onClick={() =>
                    window.open("https://slack.com/intl/pt-br/", "_blank")
                  }
                  className="cursor-pointer"
                >
                  <ExternalLink className="w-4 h-4 mr-2" />
