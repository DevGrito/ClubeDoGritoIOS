...
export const useDevAccess = (): DevAccessInfo => {
  ...
  const checkDevStatus = async () => {
    // 🔐 SECURITY: Verificar status de dev APENAS pelo papel do usuário (não por URL)
    // Parâmetros de URL como ?dev_access=true NÃO concedem acesso
    const userPapel = localStorage.getItem('userPapel');
    
    // Desenvolvedores são identificados APENAS pelo papel retornado do backend
    const isAuthenticatedDev = userPapel === 'desenvolvedor' || 
                               userPapel === 'dev' || 
                               userPapel === 'dev-admin' ||
                               userPapel === 'dev-marketing';

    // Verificar se modo dev global está ativo (controlado pelo backend)
    ...
    const finalDevInfo = {
      isDeveloper: isAuthenticatedDev || globalDevMode,
      hasDevAccess: isAuthenticatedDev,
    ...
    if (isAuthenticatedDev || globalDevMode) {
...
// 🔐 SECURITY: Verificação baseada APENAS no papel do usuário (não em URL params)
export const checkDevPermissions = (): boolean => {
  const userPapel = localStorage.getItem('userPapel');
  return userPapel === 'desenvolvedor' || 
         userPapel === 'dev' || 
         userPapel === 'dev-admin' ||
         userPapel === 'dev-marketing';
};
