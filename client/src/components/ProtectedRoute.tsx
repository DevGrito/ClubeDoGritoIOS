...
export const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ 
  ...
  useEffect(() => {
    ...
    // DUAL ROLE: Papéis que NÃO são doadores (não precisam verificar assinatura)
    // IMPORTANTE: 'leo' foi REMOVIDO desta lista porque Leo também é doador
    ...
    // DUAL ROLE: Leo e outros usuários com papel admin + doador devem verificar assinatura para rotas de doador
    const isDualRoleUser = userPapel === 'leo';
    
    console.log(`🚨 [PAYMENT DEBUG] routeName: ${routeName}, hasActiveSubscription: ${hasActiveSubscription}, subscriptionPaused: ${subscriptionPaused}, userPapel: ${userPapel}, isAdminOnlyRole: ${isAdminOnlyRole}, isDonorRoute: ${isDonorRoute}, isDualRoleUser: ${isDualRoleUser}`);
    
    // Se é uma rota de doador e não tem assinatura ativa, redirecionar
    // DUAL ROLE: Usuários com papel dual (ex: leo) precisam de assinatura para área de doador
    if (isDonorRoute && !isAdminOnlyRole && !hasActiveSubscription) {
      console.log(`🚨 [PAYMENT BLOCK] Usuário sem assinatura ativa tentando acessar área de doador (isDualRoleUser: ${isDualRoleUser})`);
