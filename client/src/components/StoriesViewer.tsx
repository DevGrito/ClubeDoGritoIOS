...
import { X, MessageCircle, Share, Heart, Copy, Check } from "lucide-react";
...
export default function StoriesViewer({ 
  ...
  // Compartilhar - abrir modal com opções
  const handleShare = async () => {
    ...
    // Pausar a história ao abrir o modal
    setIsPlaying(false);
    setShowShareModal(true);
  };

  // Funções de compartilhamento para cada rede social
  const shareUrl = window.location.href;
  const shareText = `${currentPage?.storyName || 'História inspiradora'} - ${currentPage?.title || 'Confira esta história!'}`;
  ...
  const shareToInstagram = () => {
    // Instagram não tem API de compartilhamento direto, copiar link
  ...
  const shareToTikTok = () => {
    // TikTok não tem API de compartilhamento direto, copiar link
    navigator.clipboard.writeText(shareUrl);
    setLinkCopied(true);
    setTimeout(() => setLinkCopied(false), 2000);
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: currentPage?.storyName || 'História inspiradora',
          text: currentPage?.title || 'Confira esta história!',
          url: shareUrl
        });
        setShowShareModal(false);
      } catch (error) {
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm">
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-gray-900">Compartilhar História</h3>
              <button 
                onClick={() => {
                  setShowShareModal(false);
                  setIsPlaying(true);
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              ...
              <button 
                ...
                className="flex flex-col items-center gap-2 p-3 rounded-xl hover:bg-gray-100 transition-colors"
              >
                <div className="w-14 h-14 bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600 rounded-full flex items-center justify-center">
              ...
              {typeof navigator !== 'undefined' && typeof navigator.share === 'function' && (
                ...
                >
                  ...
                  <span className="text-xs text-gray-600 font-medium">Mais</span>
                </button>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 p-3 bg-gray-100 rounded-xl">
              ...
              <button 
                onClick={copyLink}
                className={`p-2 rounded-lg transition-colors ${linkCopied ? 'bg-green-500 text-white' : 'bg-yellow-500 text-black hover:bg-yellow-600'}`}
              >
                {linkCopied ? <Check className="w-5 h-5" /> : <Copy className="w-5 h-5" />}
              </button>
            </div>
            {linkCopied && (
              <p className="text-center text-green-600 text-sm mt-2 font-medium">Link copiado!</p>
            )}
          </div>
        </div>
      )}
