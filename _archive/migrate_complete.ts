...
async function migrate() {
  try {
    ...
    // 1. Migrar indicadores (gv_mgmt_indicators)
    ...
    for (const row of indicators.rows) {
      ...
      if (exists.rows.length === 0) {
        await doClient.query(`
          ...
          (id, name, description, unit, category, active, created_at, updated_at)
          VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        `, [
          row.id, row.name, row.description, row.unit, 
          row.category, row.active, row.created_at, row.updated_at
        ]);
        indicatorsAdded++;
      }
    }
    console.log(`  ✓ ${indicatorsAdded} indicadores migrados\n`);

    // 2. Migrar indicator assignments
    ...
    console.log(`  ✓ ${assignmentsAdded} assignments migrados\n`);
