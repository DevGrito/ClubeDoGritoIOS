...
async function migrate() {
  try {
    ...
    console.log('📊 Migrando indicadores (gv_mgmt_indicators)...');
    const indicators = await replitClient.query(`
      SELECT * FROM gv_mgmt_indicators 
      WHERE id > 66 
      ORDER BY id
    `);
    ...
    for (const row of indicators.rows) {
      const exists = await doClient.query(
        'SELECT id FROM gv_mgmt_indicators WHERE id = $1',
        [row.id]
      );
      ...
      if (exists.rows.length === 0) {
        ...
        indicatorsAdded++;
        console.log(`    ✓ Indicador ${row.id}: ${row.name}`);
      }
    }
    console.log(`  Total: ${indicatorsAdded} indicadores migrados\n`);

    // 2. Migrar assignments
    ...
    console.log(`  Total: ${assignmentsAdded} assignments migrados\n`);

    // 3. Migrar dados mensais
    console.log('📊 Migrando dados mensais (gv_monthly_data)...');
    ...
    console.log(`  Total: ${dataAdded} dados mensais migrados\n`);

    // 4. Atualizar sequences
    console.log('🔄 Atualizando sequences...');
    await doClient.query(`
      SELECT setval('gv_mgmt_indicators_id_seq', 
        COALESCE((SELECT MAX(id) FROM gv_mgmt_indicators), 1), true)
    `);
    await doClient.query(`
      SELECT setval('gv_indicator_assignments_id_seq', 
        COALESCE((SELECT MAX(id) FROM gv_indicator_assignments), 1), true)
    `);
    await doClient.query(`
      SELECT setval('gv_monthly_data_id_seq', 
        COALESCE((SELECT MAX(id) FROM gv_monthly_data), 1), true)
    `);
    ...
    console.log('═══════════════════════════════════════');
    console.log(`📊 Resumo:`);
    console.log(`  • ${indicatorsAdded} indicadores adicionados`);
    console.log(`  • ${assignmentsAdded} assignments adicionados`);
    console.log(`  • ${dataAdded} dados mensais adicionados (AGO/SET PEC)`);
    console.log('═══════════════════════════════════════\n');
