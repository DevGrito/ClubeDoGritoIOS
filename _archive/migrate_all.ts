...
async function migrate() {
  try {
    ...
    for (const row of projects.rows) {
      ...
      if (exists.rows.length === 0) {
        await doClient.query(`
          ...
          (id, name, description, sector_id, workstream_id, active, created_at, updated_at)
          VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        `, [
          row.id, row.name, row.description, row.sector_id, 
          row.workstream_id, row.active, row.created_at, row.updated_at
        ]);
        projectsAdded++;
        console.log(`  ✓ Projeto ${row.id}: ${row.name}`);
      }
    }
    console.log(`  Total: ${projectsAdded} projetos migrados\n`);
    ...
    console.log('═══════════════════════════════════════════════════');
    console.log(`📊 Dados migrados do Replit → Digital Ocean:`);
    console.log(`  • ${projectsAdded} projetos`);
    console.log(`  • ${indicatorsAdded} indicadores`);
    console.log(`  • ${assignmentsAdded} assignments`);
    console.log(`  • ${dataAdded} dados mensais (incluindo AGO/SET PEC)`);
    console.log('═══════════════════════════════════════════════════\n');
