...
export const doadores = pgTable("doadores", {
  ...
  plano: text("plano").notNull(), // 📊 VALORES PERMITIDOS: 'eco', 'voz', 'grito', 'platinum'
  valor: decimal("valor", { precision: 10, scale: 2 }).notNull(),
  periodicidade: text("periodicidade").default("mensal"), // 📅 VALORES: 'mensal', 'trimestral', 'semestral', 'anual'
  stripePaymentIntentId: text("stripe_payment_intent_id"),
  stripeSubscriptionId: text("stripe_subscription_id"),
  stripeCustomerId: text("stripe_customer_id"), // 💳 ID do customer no Stripe
  status: text("status").default("pending"), // 🔄 VALORES PERMITIDOS: 'pending', 'paid', 'failed', 'cancelled'
  ...
  ativo: boolean("ativo").default(true),
  dataAdmissao: date("data_admissao"),
  dataDesligamento: date("data_desligamento"), // ✅ Campo para controle de ativo/inativo
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
...
export const doadoresExternos = pgTable("doadores_externos", {
  ...
  observacao: text("observacao"), // ex: "DOADOR ANJO"
...
export const aluno = pgTable("aluno", {
  ...
  // 🟡 Relacionamentos com familiares
...
export const updateMonitorParticipanteSchema = createInsertSchema(monitorParticipantes).omit({ id: true, monitorUserId: true, participanteId: true, createdAt: true, updatedAt: true }).partial();
